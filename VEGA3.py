...
#Backup normal recording
picam2.start_recording(H264Encoder(), "VEGA_Backup_1080p.h264")
print("1080p HD Backup Raw Recording Started...")
...
try:
    ...
    while (time.time() - start_time) < duration:
        ...
        current_second = int(elapsed_exact)
        if current_second > last_logged_second:
            print(f"Recording... T+{current_second}s | Scale: {smooth_min:.3f} to {smooth_max:.3f} | CPU: {cpu_usage}% | RAM: {ram_usage}%")
            csv_writer.writerow([current_second, round(mean_val, 4), round(smooth_min, 3), round(smooth_max, 3), cpu_usage, ram_usage])
    ...
    print("1080p Raw Video saved to VEGA_Backup_1080p.h264")
